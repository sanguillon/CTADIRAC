"""
ProvenanceDB module
manage the access to the Provenance database
"""
# imports
import json
...
from sqlalchemy import create_engine, func, \
    Integer, String, DateTime, exc, BigInteger
...
wasInformedBy_association_table\
    = Table('wasInformedBy', provBase.metadata,
            Column('internal_key', BigInteger,
                   primary_key=True, autoincrement=True),
            Column('informant', BigInteger,
                   ForeignKey("activities.internal_key")),
            Column('informed', BigInteger,
                   ForeignKey("activities.internal_key")))
...
class Activity(provBase):
    ...
    activityDescription_key = \
        Column(BigInteger, ForeignKey("activityDescriptions.internal_key"))
    activityDescription = relationship("ActivityDescription")
    # n-n relation
    wasInformedBy = \
        relationship('Activity',
                     secondary=wasInformedBy_association_table,
                     primaryjoin=internal_key ==
                         wasInformedBy_association_table.c.informed,
                     secondaryjoin=internal_key ==
                         wasInformedBy_association_table.c.informant)

    def __repr__(self):
        """
        Print method
        :return: list of ordered attributes of the class
        """
        response = ""
    ...
    def get_display_attributes(self):
        """
        get the attributes which will be displayed
        :return: list of displayed attributes of the class
        """
        response = {}
    ...
    def get_description_key(self):
        """
        get the key of the activity description
        :return: list of displayed attributes of the class
        """
...
wasDerivedFrom_association_table =\
    Table('wasDerivedFrom', provBase.metadata,
          Column('internal_key', BigInteger, primary_key=True),
          Column('generatedEntity', BigInteger,
                 ForeignKey("entities.internal_key")),
          Column('usedEntity', BigInteger,
                 ForeignKey("entities.internal_key")))
...
class Entity(provBase):
    ...
    ordered_attribute_list = ['id', 'classType', 'name', 'location',
                              'generatedAtTime', 'invalidatedAtTime',
                              'comment', 'entityDescription_id']
    ...
    entityDescription_key = \
        Column(BigInteger, ForeignKey("entityDescriptions.internal_key"))
    entityDescription = relationship("EntityDescription")
    # n-n relation
    wasDerivedFrom = \
        relationship('Entity', secondary=wasDerivedFrom_association_table,
                     primaryjoin=internal_key ==
                         wasDerivedFrom_association_table.c.usedEntity,
                     secondaryjoin=internal_key ==
...
class ValueEntity(Entity):
    ...
    # Internal key
    internal_key = Column(BigInteger, ForeignKey('entities.internal_key'),
                          primary_key=True)
...
class Used(provBase):
    ...
    usageDescription_key = Column(BigInteger,
                                  ForeignKey('usageDescriptions.internal_key'))
...
class WasGeneratedBy(provBase):
    ...
    generationDescription_key = \
        Column(BigInteger, ForeignKey('generationDescriptions.internal_key'))
...
class ActivityDescription(provBase):
    """
    Define the ActivityDescription class
    mapped to the activityDescriptions table
    """
    __tablename__ = 'activityDescriptions'
    ordered_attribute_list = ['name', 'version', 'description', 'type',
                              'subtype', 'doculink']
    other_display_attributes = ['name', 'version', 'description', 'type',
                                'subtype', 'doculink']
...
class EntityDescription(provBase):
    ...
    ordered_attribute_list = ['name', 'type', 'description',
                              'doculink', 'classType']
    other_display_attributes = ['name', 'type', 'description',
                                'doculink', 'classType']
...
class ValueDescription(EntityDescription):
    """
    Define the ValueDescription class
    mapped to the valueDescriptions table
    """
    __tablename__ = 'valueDescriptions'
    # ordered_attribute_list =
    # EntityDescription.ordered_attribute_list
    # + ['valueType','unit','ucd','utype']
    ...
    internal_key = \
        Column(BigInteger,
               ForeignKey('entityDescriptions.internal_key'), primary_key=True)
...
class DatasetDescription(EntityDescription):
    ...
    # Internal key
    internal_key = Column(BigInteger,
                          ForeignKey('entityDescriptions.internal_key'),
                          primary_key=True)
...
class UsageDescription(provBase):
    ...
    ordered_attribute_list = ['role', 'description', 'type',
                              'multiplicity', 'activityDescription_key',
                              'entityDescription_key']
    # Key
    internal_key = Column(BigInteger, primary_key=True, autoincrement=True)
    ...
    activityDescription_key = \
        Column(BigInteger,
               ForeignKey('activityDescriptions.internal_key'))
    activityDescription = relationship("ActivityDescription")
    # n-1 relation with EntityDescription
    entityDescription_key = \
        Column(BigInteger, ForeignKey('entityDescriptions.internal_key'))
...
class GenerationDescription(provBase):
    """
    Define the GenerationDescription class mapped
    to the generationDescriptions table
    """
...
class WasConfiguredBy(provBase):
    ...
    activity_key = Column(BigInteger, ForeignKey('activities.internal_key'))
    activity = relationship("Activity",
                            backref='WasConfiguredBy', uselist=False)
...
class Parameter(provBase):
    ...
    parameterDescription_key = \
        Column(BigInteger, ForeignKey("parameterDescriptions.internal_key"))
...
class ConfigFile(provBase):
    ...
    configFileDescription_key = \
        Column(BigInteger, ForeignKey("configFileDescriptions.internal_key"))
...
class ParameterDescription(provBase):
    ...
    ordered_attribute_list = ['name', 'valueType', 'description',
                              'unit', 'ucd', 'utype', 'min', 'max',
                              'default', 'options', 'activityDescription_key']
...
class ConfigFileDescription(provBase):
    """
    Define the ConfigFileDescription class
    mapped to the configFileDescriptions table
    """
    __tablename__ = 'configFileDescriptions'
    ordered_attribute_list = ['name', 'contentType', 'description',
                              'activityDescription_key']
...
class ProvenanceDB(object):
    ...
    def __init__(self):
        ...
        self.engine = \
            create_engine('postgresql://%s:%s@%s:%s/%s' % (self.dbUser,
                                                           self.dbPass,
                                                           self.dbHost,
                                                           self.dbPort,
                                                           self.dbName),
    ...
    def _sessionAdd(self, provInstance):
        ...
        :param provInstance:
        :return: result of the Add and if OK the internal_key
        """
        session = self.sessionMaker_o()
        try:
            session.add(provInstance)
            session.commit()
            res = session.query(func.max(provInstance.internal_key).
                                label('internal_key')).one()
            session.commit()
    ...
    def _dictToObject(self, table, fromDict):
        '''
        Transform teh JSON dictionary into an object
        :param table:
        :param fromDict:
        :return: the object
        '''
    ...
    def addActivityDescription(self, rowDict):
        ...
        try:
            res = \
                self.getActivityDescriptionKey(
                    activityDesc.name,
                    activityDesc.version)['Value']['internal_key']
            return S_OK({"internal_key": res})
        except Exception:
            return self._sessionAdd(row)
    ...
    def addDatasetDescription(self, rowDict):
        ...
        try:
            res = self.getEntityDescriptionKey(
                datasetDesc.name)['Value']['internal_key']
            return S_OK({"internal_key": res})
        except Exception:
            return self._sessionAdd(row)
    ...
    def addValueDescription(self, rowDict):
        ...
        try:
            res = self.getEntityDescriptionKey(
                valueDesc.name)['Value']['internal_key']
            return S_OK({"internal_key": res})
        except Exception:
            return self._sessionAdd(row)
    ...
    def getUsageDescription(self, activityDescription_key, role):
        ...
        try:
            usageDescription = session.query(UsageDescription) \
                .filter(UsageDescription.activityDescription_key ==
                        activityDescription_key,
            ...
            return S_OK({'internal_key': usageDescription.internal_key,
                         'entityDescription_key':
                             usageDescription.entityDescription_key})
        except NoResultFound, e:
            return S_OK()
        finally:
            session.close()
    ...
    def getGenerationDescription(self, activityDescription_key, role):
        ...
        try:
            generationDescription = session.query(GenerationDescription) \
                .filter(GenerationDescription.activityDescription_key ==
                        activityDescription_key) \
    ...
    def getParameterDescription(self, activityDescription_key, parameter_name):
        ...
        try:
            parameterDescription = session.query(ParameterDescription) \
                .filter(ParameterDescription.activityDescription_key ==
                        activityDescription_key,
    ...
    def getConfigFileDescription(self, activityDescription_key,
                                 configFile_name):
        '''
        ...
        try:
            configFileDescription = session.query(ConfigFileDescription) \
                .filter(ConfigFileDescription.activityDescription_key ==
                        activityDescription_key,
    ...
    def getActivityDescriptionKey(self, activityDescription_name,
                                  activityDescription_version):
        """
          Get getActivityDescriptionKey
          :param activityDescription_name:
          :param activityDescription_version:
          :return: ActivityDescription.internal_key
        """
        session = self.sessionMaker_o()
        try:
            activityDescription_list = session.query(ActivityDescription) \
                .filter(ActivityDescription.name ==
                        activityDescription_name) \
                .filter(ActivityDescription.version ==
                        activityDescription_version) \
                .all()
            activityDescription_last = activityDescription_list[-1]
            return S_OK({'internal_key':
                         activityDescription_last.internal_key})
    ...
    def getUsageDescriptionKey(self, usageDescription_activity,
                               usageDescription_entity, usageDescription_role):
        """
          Get getUsageDescriptionKey
          :param usageDescription_activity:
          :param usageDescription_entity:
          :param usageDescription_role:
          :return: UsageDescription.internal_key
        """
        session = self.sessionMaker_o()
        try:
            usageDescription_list = session.query(UsageDescription) \
                ...
                .filter(UsageDescription.entityDescription_key ==
                        usageDescription_entity) \
    ...
    def getGenerationDescriptionKey(self, generationDescription_activity,
                                    generationDescription_entity,
                                    generationDescription_role):
        """
          Get getGenerationDescriptionKey
          :param generationDescription_activity:
          :param generationDescription_entity:
          :paramgenerationDescription_role:
          :return: GenerationDescription.internal_key
        """
        session = self.sessionMaker_o()
        try:
            generationDescription_list = session.query(GenerationDescription) \
                ...
                .filter(GenerationDescription.entityDescription_key ==
                        generationDescription_entity) \
                .filter(GenerationDescription.role ==
                        generationDescription_role) \
                .all()

            generationDescription_last = generationDescription_list[-1]
            return S_OK({'internal_key':
                         generationDescription_last.internal_key})
        except NoResultFound, e:
            return S_OK()
        finally:
            session.close()
